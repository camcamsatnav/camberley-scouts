...
const ToastTrigger = () => {
  ...
  return (
    <div>
      <button onClick={() => toast.success('Operation succeeded')} data-testid='trigger-success'>
        Success
      </button>
      <button onClick={() => toast.success('Operation succeeded', 'Success Title')} data-testid='trigger-success-title'>
        Success with title
      </button>
      <button onClick={() => toast.error('Something went wrong')} data-testid='trigger-error'>
        Error
      </button>
      <button onClick={() => toast.error('Something went wrong', 'Error Title')} data-testid='trigger-error-title'>
        Error with title
      </button>
      <button onClick={() => toast.info('Here is some info')} data-testid='trigger-info'>
        Info
      </button>
      <button onClick={() => toast.warning('Be careful')} data-testid='trigger-warning'>
        Warning
      </button>
    </div>
  );
...
it('should throw if useToast is used outside of ToastProvider', () => {
  ...
  expect(() => render(<ThrowingComponent />)).toThrow('useToast must be used within a ToastProvider');
});
