...
const createSchema = (t: TFunction) =>
  z.object({
    name: z
      .string()
      .min(1, t('aboutUs.contact.form.name.required'))
      .max(100, t('aboutUs.contact.form.name.maxLength')),
    email: z.email(t('aboutUs.contact.form.email.invalid')),
    phone: z
      .string()
      .regex(UK_PHONE_REGEX, t('aboutUs.contact.form.phone.invalid'))
      .or(z.literal(''))
      .optional(),
    query: z.string().min(1, t('aboutUs.contact.form.query.required')),
    message: z
      .string()
      .min(1, t('aboutUs.contact.form.message.required'))
      .max(2000, t('aboutUs.contact.form.message.maxLength')),
    sendCopy: z.boolean(),
  });
...
export const ContactForm = ({ defaultQuery }: ContactFormProps) => {
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data) => {
    await submitContactForm({
      ...
      phone: data.phone,
      recipientType:
        data.query as (typeof RecipientTypes)[keyof typeof RecipientTypes],
      body: data.message,
  ...
  return (
    <div className='contact-form' data-testid='contact-form'>
      <span className='contact-form__title' data-testid='contact-form-text'>
      ...
      >
        ...
        <FormControl required error={!!errors.query}>
          <InputLabel id='query-type-label'>
            {t('aboutUs.contact.form.query.label')}
          </InputLabel>
          <Controller
            name='query'
            control={control}
            render={({ field }) => (
              <Select
                ...
                label={t('aboutUs.contact.form.query.label')}
                fullWidth
              >
                <MenuItem value={RecipientTypes.GENERAL}>
                  {t('aboutUs.contact.form.query.general')}
                </MenuItem>
                <MenuItem value={RecipientTypes.BEAVERS}>
                  {t('aboutUs.contact.form.query.beavers')}
                </MenuItem>
                <MenuItem value={RecipientTypes.CUBS}>
                  {t('aboutUs.contact.form.query.cubs')}
                </MenuItem>
                <MenuItem value={RecipientTypes.SCOUTS}>
                  {t('aboutUs.contact.form.query.scouts')}
                </MenuItem>
                <MenuItem value={RecipientTypes.VOLUNTEER}>
                  {t('aboutUs.contact.form.query.volunteer')}
                </MenuItem>
              </Select>
            )}
          />
        ...
        />
        <FormControlLabel
          control={<Checkbox {...register('sendCopy')} />}
          label={t('aboutUs.contact.form.copy.label')}
        />
