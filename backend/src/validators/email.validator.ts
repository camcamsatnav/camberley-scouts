...
const UK_PHONE_REGEX = /^(((\+44\s?\d{4}|\(?0\d{4}\)?)\s?\d{3}\s?\d{3})|((\+44\s?\d{3}|\(?0\d{3}\)?)\s?\d{3}\s?\d{4})|((\+44\s?\d{2}|\(?0\d{2}\)?)\s?\d{4}\s?\d{4}))(\s?#(\d{4}|\d{3}))?$/;
...
export const emailRequestSchema = z.object({
  ...
  phone: z.string().regex(UK_PHONE_REGEX, 'Invalid phone number').or(z.literal('')).optional(),
